...
export class SearchComponent implements OnDestroy, OnInit {
   ...
   * @param searchData the search data from the form
   */
  public onSubmitSearch(searchData: any) {
    // Update the search options with the new data
    const currentOptions = this.searchOptions$.value;
    if (currentOptions) {
      const newOptions = Object.assign({}, currentOptions, searchData);
      this.searchOptions$.next(new PaginatedSearchOptions(newOptions));
      this.retrieveSearchResults(new PaginatedSearchOptions(newOptions));
    }
  }
