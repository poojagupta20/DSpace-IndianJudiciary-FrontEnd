...
export class FileSectionComponent implements OnInit {
  ...
  viewFile(file: Bitstream) {
    if (this.item && this.item.uuid) {
      const itemUuid = this.item.uuid; // Extract item UUID dynamically
      console.log("uuid", itemUuid);
      console.log(this.item)
      const viewerUrl = `/viewer/${itemUuid}`; // Construct viewer URL using item UUID
      
      console.log("Navigating to viewer:", viewerUrl);
      this.router.navigate([viewerUrl]); // Open in the same app
    } else {
      console.error("Item UUID not found. Cannot navigate.");
    }
  }
}
