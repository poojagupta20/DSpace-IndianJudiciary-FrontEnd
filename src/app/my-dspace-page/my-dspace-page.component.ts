...
import { FilterMediaService } from '../core/serachpage/filter-media.service';
import { ItemTranscriptionService } from '../core/serachpage/item-transcription.service';
...
export class MyDSpacePageComponent implements OnInit {
  ...
  constructor(private service: SearchService,
              ...
              private filterMediaService: FilterMediaService,
              private itemTranscriptionService: ItemTranscriptionService) {
  ...
  ngOnInit(): void {
    ...
    // Trigger filter-media API
    this.filterMediaService.triggerFilterMedia().subscribe({
      next: (res) => console.log('Filter Media API response:', res),
      error: (err) => console.error('Filter Media API error:', err),
    });
    
    // Get latest item and upload transcription
    this.itemTranscriptionService.getLatestItemAndUploadTranscription().subscribe({
      next: (res) => console.log('Transcription upload response:', res),
      error: (err) => console.error('Transcription upload error:', err),
    });
    
    console.log('MyDSpacePageComponent initialized');
  }
}