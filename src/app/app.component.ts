...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.isAuthBlocking$ = this.store.pipe(select(isAuthenticationBlocking), distinctUntilChanged())

    this.dispatchWindowSize(this._window.nativeWindow.innerWidth, this._window.nativeWindow.innerHeight)

    // 🔥 FIX: Initialize authentication on app startup
  ...
  private initializeAuthentication(): void {
    if (isPlatformBrowser(this.platformId)) {
      // Check for existing authentication token and restore state
      this.authService.checkAuthenticationToken()

      // Start tracking token expiration
      this.authService.trackTokenExpiration()

      console.log("Authentication initialized on app startup")
    }
  }
  ...
  private trackIdleModal() {
    ...
    isIdle$.pipe(withLatestFrom(isAuthenticated$)).subscribe(([userIdle, authenticated]) => {
